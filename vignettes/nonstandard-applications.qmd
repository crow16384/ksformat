...
Numeric display formatting now has its own parser syntax. Put the pattern in
the VALUE header and keep the block otherwise empty except for optional
...
## When to use `fnew()` anyway

`fnew()` remains the most concise way to create a report-formatting object when
you already know you want a display pattern. `fparse()` is useful when the rule
needs to be stored as text, versioned in a spec, or generated from another
tool.
