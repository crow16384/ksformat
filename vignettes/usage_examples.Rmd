...
You can also use the labels returned by `franges()` as the full category set
for `tidyr::complete()`, without converting the column to a factor first.
...
If a format is already defined, you can inspect its label values directly from
the stored mappings. For this `aesev` example, the labels are plain character
values, so `unique()` gives the full set without needing `factor()`.
...
`flevels()` returns a tidy two-column lookup (`value`, `label`) from a
`ks_format` object (or a registered format name). This is handy when you want
to inspect, join, or export the exact mapping table used by a format.
...
Because the result is a regular `data.frame`, you can filter/select with your
preferred tools:
...
```{r flevels-cleanup, include=FALSE}
fclear()
```

## Example 31: Numeric Pattern Formatting

Numeric pattern formats are useful for continuous values such as currency,
percentages, and fixed-precision displays.

Define a currency format with grouping and two decimals:

```{r numeric-pattern-currency}
fnew(
  "$%,.2f",
  .missing = "NO DATA",
  .other = "INVALID",
...
Percent-like suffixes are supported via literal text around the numeric token:

```{r numeric-pattern-percent}
...
```{r numeric-pattern-cleanup, include=FALSE}
fclear()
```
