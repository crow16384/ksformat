...
## Example 21: Consistent Data-Driven Formats with `reverse`

When building formats from data (e.g., a data frame with 1000+ rows), you
use `setNames()` to create a named vector. By default, `fnew()` treats
...
This inconsistency is confusing for data-driven formats. The `reverse`
parameter solves it: set `reverse = FALSE` to use the natural
`c(key = value)` direction for **all** types.
...
### Same `setNames(values, keys)` pattern for both types

With `reverse = FALSE`, both formats use the **identical** calling style —
names are input keys, values are output objects:

```{r reverse-date}
# Date lookup
fnew(
  setNames(as.Date(dm$RFICDTC, format = "%Y-%m-%d"), keys),
  .other      = NA,
  type        = "Date",
  ignore_case = TRUE,
  name        = "icdtn"
...
# Character lookup — same setNames(values, keys) pattern!
fnew(
  setNames(dm$RFICDTC, keys),
  .other      = "NOT FOUND",
  type        = "character",
  ignore_case = TRUE,
  reverse     = FALSE,
...
The only difference is the `reverse = FALSE` for the character format.
For Date formats, `reverse` defaults to `FALSE` automatically, so you
don't need to specify it.

### When to use the default (reversal on)

The default reversal (`reverse = NULL`) preserves the standard R convention
...
| Use case | Style | `reverse` |
|:--------------------------|:--------------------------------------|:----------|
| Data-driven (any type)    | `setNames(values, keys)`              | `FALSE`   |
| Hand-written (char/num)   | `c(Label = "Code")` or `"Code" = "Label"` | `NULL` (default) |
| Value types (`Date`, etc.)| `setNames(values, keys)`              | `NULL` (default = `FALSE`) |
