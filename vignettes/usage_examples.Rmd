...
### Date lookup (value type — no reversal)

Because `type = "Date"`, `fnew()` treats `setNames(dates, keys)` literally:
names are input keys, values are output Date objects.

```{r reverse-date}
date_vals <- as.Date(dm$RFICDTC, format = "%Y-%m-%d")

fnew(
  setNames(date_vals, keys),
  .other      = NA,
  type        = "Date",
  ignore_case = TRUE,
  name        = "icdtn"
...
For `type = "character"`, the R convention `c(Label = "Code")` is expected,
so names are *labels* and values are *codes*. `fnew()` reverses the pairs
internally.

Because **we want** the mapping `key → date-string`, we must put the
*date-strings* (labels) in the **names** position and the *keys* (codes) in
the **values** position:

```{r reverse-char}
fnew(
  setNames(keys, dm$RFICDTC),
  .other      = "NOT FOUND",
  type        = "character",
  ignore_case = TRUE,
  name        = "icdtc"
...
### Equivalent explicit-argument style (no reversal)

When you pass individual `key = value` arguments (not a named vector),
there is no reversal regardless of type — the name is always the input key:
...
* Named-vector reversal only affects `c(...)` / `setNames()` style input
  for character and numeric formats.
* Value-type formats (`Date`, `POSIXct`, `logical`) always use the natural
  `c(key = value)` direction — no reversal.
* If you find the reversal confusing, use explicit `key = "label"` arguments
