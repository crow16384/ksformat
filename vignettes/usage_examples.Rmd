...
fnew(
  setNames(SV$SVSTDTC, paste(SV$USUBJID, SV$VISITNUM, sep = "|")),
  .other  = "NOT FOUND",
  name    = "svdtc",
  type    = "character",
  ignore_case = TRUE
...
## Example 21: Named Vector Direction and the `reverse` Convention

When you pass a named vector to `fnew()`, the direction of the name-value
mapping depends on the output **type**. Understanding this is key to avoiding
surprises.

### The rule

| Output type | Named vector convention | `reverse` | Reason |
|:---------------------|:-------------------------------------|:----------|:-----------------------------------------------|
| `"character"` / `"numeric"` | `c(Label = "Code")` — R idiom | Yes | Follows `factor()` / `setNames()` convention |
| Value types (`"Date"`, `"POSIXct"`, `"logical"`) | `c(Code = value)` — natural | No | Non-character objects cannot serve as names |

For **character / numeric** formats, `fnew()` internally reverses the
named vector so that `c(Male = "M")` becomes the mapping `"M" → "Male"`.
This matches R's standard `factor(x, levels, labels)` pattern where the
*label* sits in the name position.

For **value-type** formats (`Date`, `POSIXct`, `logical`), no reversal
happens because the *values* are rich R objects that cannot be used as
character names. Here you write the mapping in its natural direction:
`c("M" = as.Date("2021-01-01"))`.

### Clinical-data example

Suppose we have a demographics dataset and need two lookup formats from the
same data — one returning Date objects and one returning character strings:
...
dm <- data.frame(
  ...
  RFICDTC = c("2021-02-09T08:05", "2021-08-13T07:50", "2022-01-17T08:02"),
  stringsAsFactors = FALSE
)
...
### Practical: Use explicit arguments (no reversal confusion)

The **easiest and clearest way** to create formats from data is to use explicit
`key = value` arguments. This style is never reversed, regardless of type:

```{r reverse-setup-explicit}
# Create lookup mappings from dm
mapping_keys <- paste(dm$USUBJID, dm$SUBJID, sep = "|")
mapping_dates <- as.Date(dm$RFICDTC, format = "%Y-%m-%d")
mapping_strings <- dm$RFICDTC

# Date lookup — use explicit key = Date pairs
# (Same calling style as character below)
fnew(
  "SUBJ-001|001" = mapping_dates[1],
  "SUBJ-002|002" = mapping_dates[2],
  "SUBJ-003|003" = mapping_dates[3],
  .other      = NA,
  type        = "Date",
  ignore_case = TRUE,
  name        = "icdtn"
...
# Returns native Date objects
result_date <- fputk("SUBJ-001", "001", format = "icdtn")
result_date
class(result_date)

fclear()

# Character lookup — use explicit key = string pairs
# (IDENTICAL calling style)
fnew(
  "SUBJ-001|001" = mapping_strings[1],
  "SUBJ-002|002" = mapping_strings[2],
  "SUBJ-003|003" = mapping_strings[3],
  .other      = "NOT FOUND",
  type        = "character",
  ignore_case = TRUE,
  name        = "icdtc"
...
# Returns character strings
result_char <- fputk("SUBJ-001", "001", format = "icdtc")
result_char
class(result_char)
...
**Key insight:** Both Date and character formats use the **identical calling style**
(`key = value` arguments). Names are always input keys, values are always output 
objects/labels. No reversal, no confusion.

### When named-vector reversal matters (advanced)

If you do pass a named vector, reversal applies differently by type:

```{r reverse-named-vec}
# Named vector: c(Label = "Code") — R idiom
sex_codes <- c(Male = "M", Female = "F")

# For character type: reversal applies
# c(Male = "M") becomes "M" -> "Male"
fnew(sex_codes, name = "sex")
fput(c("M", "F"), "sex")

fclear()

# For value type: no reversal
# c("M" = as.Date(...)) stays as "M" -> Date
date_lookup <- c("M" = as.Date("2021-01-01"), "F" = as.Date("2021-02-01"))
fnew(date_lookup, type = "Date", name = "mf_dates")
fput(c("M", "F"), "mf_dates")
...
* **In practice**, use explicit `key = value` arguments — same style for all types,
  no reversal surprises.
* Named-vector reversal is an advanced detail that only matters if you pass
  `c(...)` or `setNames()` to `fnew()`. Avoid it unless you have a specific reason.
