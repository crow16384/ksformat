...
### 9.6 What about numeric display patterns?

The numeric pattern form of [`fnew()`](../reference/fnew.md) is still a
direct constructor feature. There is no dedicated
[`fparse()`](../reference/fparse.md) syntax for a display pattern such
as `fnew("$%,.2f", type = "numeric")` yet. If you need a textual
definition for a numeric lookup, use a standard `VALUE` block; if you
need a formatted numeric display, keep using
[`fnew()`](../reference/fnew.md).
