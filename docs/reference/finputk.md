...
Convenience wrapper around an INVALUE lookup that pastes multiple
vectors together into a composite label before reverse lookup. Mirrors
\[fputk()\] on the invalue side, for INVALUE formats built with
composite labels such as \`fmap(paste(col1, col2, sep = "\|"), codes)\`.
...
- ...:
  ...
  Vectors to paste together into a composite label. All vectors are
...
- na_as_string:
  ...
  composite label (restored to \`NA_character\_\` after the \[paste()\]
  step) so the invalue's \`missing_value\` applies. If \`TRUE\`, the
  literal string \`"NA"\` produced by \[paste()\] is kept, which is
  useful when the invalue was built with composite labels via
...
(numeric / integer → numeric, character → character, logical → logical).
